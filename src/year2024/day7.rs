...
pub fn run(input: &str) -> Result<Solution> {
    let equations = input.lines().map( |line| line.try_into()).collect::<Result<Vec<_>>>()?;
    let part1 = sum_of_valid_test_values(&equations, &vec![Operation::Add, Operation::Multiply]);
    let part2 = sum_of_valid_test_values(&equations, &vec![Operation::Add, Operation::Multiply, Operation::Concatenate]);
